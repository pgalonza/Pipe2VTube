...
def test_performance():
    ...
    print(f"Processed {num_frames} frames in {total_time:.4f} seconds")
    print(f"Average time per frame: {avg_time_per_frame*1000:.4f} ms")
    
    # Should be well under 10ms per frame for real-time performance
    # (30 FPS = 33.3ms per frame, 60 FPS = 16.6ms per frame)
    assert avg_time_per_frame < 0.01, (
    ...
    print(f"Parameters processed: {stats['parameters_processed']}")
    print(f"Parameters sent: {stats['parameters_sent']}")
    print(f"Parameters cached: {stats['parameters_cached']}")
    ...
    if stats['parameters_processed'] > 0:
        ...
        print(f"Parameter reduction ratio: {reduction_ratio:.2%}")
